...
class MeshDrawer {
	...
	setTexture(img, textureUnit = gl.TEXTURE0, uniformName = 'tex') {
		...
		gl.useProgram(this.prog);
		const sampler = gl.getUniformLocation(this.prog, uniformName);
			...
			void main()
			{
				if(showTex && enableLighting){
					...
					if (diffuseFactor > 0.0) {
						spec = pow(max(dot(viewDir, reflectDir), 0.0), specular); 
					}
					...
					vec4 texColor2 = texture2D(tex2, v_texCoord); //second texture for task2

					vec3 blendedColor = mix(texColor.rgb, texColor2.rgb, texOpacity);
        			gl_FragColor = vec4(blendedColor * lighting, texColor.a);
				
				}
				else if(showTex){
					vec4 texColor = texture2D(tex, v_texCoord);   //First texture
					vec4 texColor2 = texture2D(tex2, v_texCoord); // Second texture
					gl_FragColor = mix(texColor, texColor2, texOpacity);
				}
				else{
